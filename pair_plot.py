    ...
    for y, course_y in enumerate(courses):
        for x, course_x in enumerate(courses):
            if (course_y != course_x):
                x_data = data.houses["all_houses"][course_x]
                y_data = data.houses["all_houses"][course_y]

                size = min(len(x_data), len(y_data))
                x_data = x_data[:size]
                y_data = y_data[:size]
                axs[y, x].scatter(x_data, y_data)
                # axs[y, x].scatter(data.houses["all_houses"][course_y], bins=100, alpha=0.5)
            # else:
            #     for house in houses:
